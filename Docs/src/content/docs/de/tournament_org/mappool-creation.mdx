...
To create a mappool, use the `/mappool_create` command in an `admin` or `mappool` channel that you had designated during the [tournament creation](#tournament-creation) section. The only parameter is the target\_sr, as the command will guide you through each part of the mappool creation process.
