...
While you necessarily shouldn't need to create them, you may create one for players/teams by using the same discord command they use, which is `/tournament_qualifier`. You will require providing the `target` parameter, which is the player/team you are creating the matchup for. This command is useful if the player/team schedule the qualifier at a time they did not want.
