...
export function extractTargetText (m: Message | ChatInputCommandInteraction, index: number, commandName: string) {
    if (m instanceof ChatInputCommandInteraction) {
        ...
        if (m.options.getString(commandName))
            return m.options.getString(commandName)?.trim();
