...
async function run (m: Message | ChatInputCommandInteraction) {
    ...
    const scores = matchups
        ...
        .map(score => {
            ...
            const team = teams.find(t => t.members.some(member => member.ID === score.user?.ID))?.name ?? "N/A";
            const mp = matchups.find(matchup => matchup.sets?.some(set => set.maps?.some(map => map.scores?.some(s => s.ID === score.ID)) ?? false))?.mp ?? "N/A";

            // Obtain map name
            const map = maps.find(matchupMap => matchupMap.scores?.some(s => s.ID === score.ID));
            const mapName = mapNames.find(name => name === `${map?.map?.slot?.acronym}${map?.map?.order}`) ?? "N/A";
