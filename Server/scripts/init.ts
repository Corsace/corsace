...
readFile(configPath, "utf-8").then(async (data) => {
    ...
    const getInfo = async (question: string, cb: (result: string) => void, initialText?: string) => {
        return new Promise<void>(resolve => {
            ...
            rl.question(question, async (result: string) => {
                cb(result);
                await writeFile(configPath, JSON.stringify(configData, null, 4));
                rl.close();
                resolve();
            });
        });
    ...
    if (!configData.osu.v1.apiKey || configData.osu.v1.apiKey === "obtain from https://osu.ppy.sh/p/api/")
        await getInfo("osu! API V1 key: ", (apiKey: string) => configData.osu.v1.apiKey = apiKey, `osu! API key is not set.\nYou can get it from the ${greenBoldBackground}Legacy API${resetColor} section at the bottom of ${linkHighlight}"https://osu.ppy.sh/home/account/edit"${resetColor}.`);
    ...
    if (!configData.osu.bancho.username || !configData.osu.bancho.ircPassword || configData.osu.bancho.username === "bot account username, use your own for dev" || configData.osu.bancho.ircPassword === "obtain from https://osu.ppy.sh/home/account/edit#irc") {
        await getInfo("osu! IRC username: ", (username: string) => configData.osu.bancho.username = username, `osu! IRC username and password are not set.\nYou can get your password from the ${greenBoldBackground}Legacy API${resetColor} section at the bottom of ${linkHighlight}"https://osu.ppy.sh/home/account/edit"${resetColor}.`);
    ...
    if (!configData.discord.token || configData.discord.token === "bot token")
        await getInfo("Discord bot token: ", (token: string) => configData.discord.token = token, `Discord bot token is not set.\nCreate an application at ${linkHighlight}"https://discord.com/developers/applications"${resetColor}\nGo to the ${greenBoldBackground}Bot${resetColor} section\nPress "Reset Token"\nCopy the token, and paste it here.\n${yellowBoldBackground}Do not share this token with anyone.\nPlease make sure to provide the bot "Message Content Intent" and "Server Members" intents as well!${resetColor}`);
    if (!configData.discord.clientId || !configData.discord.clientSecret || configData.discord.clientId === "bot ID" || configData.discord.clientSecret === "bot secret") {
        ...
        await getInfo(`Invite the bot to your server by using this link: ${linkHighlight}https://discordapp.com/oauth2/authorize?&client_id=${configData.discord.clientId}&scope=bot&permissions=8${resetColor}\nPress enter when you're done.`, () => {
            // Do nothing
        });
    }
    ...
    // Migration stuff
    await getInfo(`Start the database service from docker? ${redBackground}(Recommended)${resetColor} (y/n) Default: y `, (isDocker: string) => {
    ...
    await getInfo("Do you want to run the migration? (y/n) Default: y ", (ready: string) => {
        ...
        console.log(`\n${yellowBackground}Running migration...${resetColor}`);
        exec("npm run migration");
        console.log(`\n${greenBackground}Migration complete!${resetColor}`);
    });
    ...
    await getInfo("Will you be working on the MCA/AYIM project? (y/n) Default: n ", async (isMCA: string) => {
        ...
        if (!configData.discord.roles.mca.standard || !configData.discord.roles.mca.taiko || !configData.discord.roles.mca.fruits || !configData.discord.roles.mca.mania || !configData.discord.roles.mca.storyboard || configData.discord.roles.mca.standard === "role ID")
            ...
            }, `Discord MCA role ID is not set.\nCreate one if you haven't already\nGet the ID by right-clicking the role and selecting "Copy Role ID".\n${yellowBoldBackground}This role is used for all modes.${resetColor}`);

        // Check BN account and prompt user to enter it if it's not set
        if (!configData.bn.username || !configData.bn.secret)
            await getInfo(`Do you have interop access to the ${linkHighlight}https://bn.mappersguild.com/${resetColor} site? (y/n) Default: n `, async (hasBN: string) => {
        ...
        await getInfo(`Do you want to set up an initial MCA? ${redBackground}(Recommended)${resetColor} (y/n) Default: y `, (initMCA: string) => {
    ...
    await getInfo("Will you be working on the tournaments infrastructure? (y/n) Default: n ", async (isTournament: string) => {
        ...
        if (!configData.s3.clients.r2.hostname || configData.s3.clients.r2.hostname === "<cloudflare account id>.r2.cloudflarestorage.com")
            ...
            }, `Cloudflare R2 account ID is not set.\nCreate a Cloudflare account if you haven't already from ${linkHighlight}https://dash.cloudflare.com/?to=/:account/r2${resetColor} and enable a plan (You will not exceed free limits)\nCopy and paste your account ID shown on the right of the ${greenBoldBackground}R2 Overview${resetColor} page.\n${yellowBoldBackground}Do not share this information with anyone.${resetColor}`);
        if (!configData.s3.buckets.mappacks.publicUrl || configData.s3.buckets.mappacks.publicUrl === "set custom domain or enable r2.dev url in bucket settings")
            ...
            }, `Cloudflare R2 mappacks bucket URL is not set.\nCreate a bucket called ${greenBoldBackground}mappacks${resetColor} if you haven't already from the ${greenBoldBackground}R2 Overview${resetColor} page\nGo to the bucket settings and click "Allow Access" in the ${greenBoldBackground}R2.dev subdomain${resetColor} section\nCopy the URL and paste it here.\n${yellowBoldBackground}Do not share this information with anyone.${resetColor}`);
        await getInfo("", () => {
            // Do nothing
        }, `If you have already created a mappacks-temp bucket, press enter.\n\nOtherwise, create a bucket called ${greenBoldBackground}mappacks-temp${resetColor} from the ${greenBoldBackground}R2 Overview${resetColor} page\nGo to the bucket settings and click "Add rule" in the ${greenBoldBackground}Object lifecycle rules${resetColor} section\nSet the rule to ${greenBoldBackground}Delete after 1 day${resetColor}\nPress enter when you're done.`);
        if (!configData.s3.buckets.teamAvatars.publicUrl || configData.s3.buckets.teamAvatars.publicUrl === "set custom domain or enable r2.dev url in bucket settings")
            ...
            }, `Cloudflare R2 team avatars bucket URL is not set.\nCreate a bucket called ${greenBoldBackground}tournament-team-avatars${resetColor} if you haven't already from the ${greenBoldBackground}R2 Overview${resetColor} page\nGo to the bucket settings and click "Allow Access" in the ${greenBoldBackground}R2.dev subdomain${resetColor} section\nCopy the URL and paste it here.\n${yellowBoldBackground}Do not share this information with anyone.${resetColor}`);

        // Set up an initial tournament via npm run init:tournament
        await getInfo(`Do you want to set up an initial tournament? ${redBackground}(Recommended)${resetColor} (y/n) Default: y `, (initTournament: string) => {
            if (initTournament.toLowerCase() !== "n")
                exec("npm run init:tournament");
        });
    });
}).catch((err) => console.error(`${redBoldBackground}Error reading config file:\n${err}.${resetColor}\n\nPlease create and copy the default config file shown in ./config/default.json to ${configPath} and try again.`));