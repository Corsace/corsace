...
import { TournamentStageState, TournamentRoundState } from "koa";
import { CorsaceContext } from "../../../corsaceRouter";

export async function getMatchups (ctx: CorsaceContext<{ matchups: MatchupList[] }, TournamentStageState | TournamentRoundState>) {
    const parentKey = "round" in ctx.state ? "round" : "stage";
    const parentValue = ctx.state[parentKey]!;

    let matchups: MatchupWithRelationIDs[] = await Matchup
        .createQueryBuilder("matchup")
        .innerJoin(`matchup.${parentKey}`, parentKey)
        .leftJoinAndSelect("matchup.referee", "referee")
        .leftJoinAndSelect("matchup.streamer", "streamer")
        .leftJoinAndSelect("matchup.potentialFor", "potentialFor")
        .where(`${parentKey}.ID = :parentID`, { parentID: parentValue.ID })
...
export async function getMappools (ctx: CorsaceContext<{ mappools: Mappool[] }, TournamentStageState | TournamentRoundState>) {
    const parentKey = "round" in ctx.state ? "round" : "stage";
    const parentValue = ctx.state[parentKey]!;
    ...
    const tournament = await Tournament
        ...
        .where("tournament.ID = :tournamentID", { tournamentID: ctx.state.stage.tournament.ID })
        .getOne();
    ...
    if (parentKey === "round")
        mappoolQ = mappoolQ
            .innerJoinAndSelect("mappool.round", "round");

    const mappools = await mappoolQ
        .andWhere(`${parentKey}.ID = :parentID`, { parentID: parentValue.ID })
        .getMany();
...
export async function getScores (ctx: CorsaceContext<{ scores: MatchupScore[] }, TournamentStageState | TournamentRoundState>) {
    const parentKey = "round" in ctx.state ? "round" : "stage";
    const parentValue = ctx.state[parentKey]!;
    ...
    if(parentKey === "round") {
        rawScoresQ = rawScoresQ
            .innerJoin("matchup.round", "round");
    }
    const rawScores = await rawScoresQ
        .andWhere(`${parentKey}.ID = :parentID`, { parentID: parentValue.ID })
        .getRawMany();
