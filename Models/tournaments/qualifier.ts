...
export class Qualifier extends BaseEntity {
    ...
    @ManyToOne(
        () => Tournament,
        (tournament) => tournament.qualifiers
    )
    tournament!: Tournament;

    @ManyToOne(
        () => Mappool,
        (mappool) => mappool.qualifiers
    )
    mappool!: Mappool;

    @OneToMany(
        () => MatchPlay,
        (score) => score.qualifier
    )
    scores?: MatchPlay[];
    ...
    public getInfo = async function(this: Qualifier): Promise<QualifierInfo> {
        const info: QualifierInfo = {
            ID: this.ID,
            teams: this.teams
                ? await Promise.all(this.teams.map((team) => team.getInfo()))
                : undefined,
            scores: this.scores
                ? await Promise.all(this.scores.map((score) => score.getInfo()))
                : undefined,
            time: this.time,
