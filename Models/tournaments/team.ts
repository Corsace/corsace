...
export class Team extends BaseEntity {
    ...
    public getMembers = async function(
        this: Team,
        allMembers?: User[]
    ): Promise<User[]> {
        return allMembers
            ? allMembers.filter((m) => m.team && m.team === this)
            : await User.find({ team: this });
    };

    public computeBWS = async function(
        this: Team,
        save = true,
        computeRankOnUpdate = true
    ): Promise<number> {
        const members = await this.getMembers();
        const ratings = (
            await Promise.all(members.map((m) => m.getBWS()))
        ...
        if (ratings.length > 4) {
            if (ratings.length % 2 !== 0) ratings.splice(0, 1);
    ...
    public computeRank = async function(
        this: Team,
        save = true,
        computeAllRanksOnUpdate = true
    ): Promise<number | null> {
        const oldRank = this.rank;
        const eligibleamount = this.tournament.getEligibleAmount();
        this.rank = this.isEligible()
            ? (await Team.createQueryBuilder("team")
                .where(
                    `averageBWS < ${this.averageBWS} AND membersAmount >= ${eligibleamount}`
                )
                .getCount()) + 1
            : null;
        if (save) {
    ...
    public getInfo = async function(this: Team): Promise<TeamInfo> {
        const info: TeamInfo = {
            ...
            members: await Promise.all(
                this.members.map((member) => member.getInfo())
            ),
            role: this.role,
            ...
            matches: this.matches
                ? await Promise.all(
                    this.matches.map((match) => match.getInfo())
                )
                : undefined,
            mapsWon: this.mapsWon
                ? await Promise.all(this.mapsWon.map((map) => map.getInfo()))
                : undefined,
            setsWon: this.setsWon
                ? await Promise.all(this.setsWon.map((set) => set.getInfo()))
                : undefined,
            matchesWon: this.matchesWon
                ? await Promise.all(
                    this.matchesWon.map((match) => match.getInfo())
                )
                : undefined,
            matchesFirst: this.matchesFirst
                ? await Promise.all(
                    this.matchesFirst.map((match) => match.getInfo())
                )
                : undefined,
        };
        return info;
    ...
    public getSeed = function(this: Team): "A" | "B" | "C" | "D" | null {
        if (!this.isEligible() || !this.rank) return null;
        return "A";
    };

    public getTeamInvites = async function(
        this: Team
    ): Promise<TeamInvitation[]> {
        return await TeamInvitation.find({ team: this });
    };

    public getPendingTeamInvites = async function(
        this: Team
    ): Promise<TeamInvitation[]> {
