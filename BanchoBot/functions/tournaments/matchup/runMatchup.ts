...
async function runMatchupListeners (matchup: Matchup, mpLobby: BanchoLobby, mpChannel: BanchoChannel, invCollector?: InteractionCollector<any>, refCollector?: InteractionCollector<any>, auto = false) {
        ...
        if (message.user.ircUsername === "BanchoBot") {
            if (
                ...
                setTimeout(async () => {
                    ...
                    await mpChannel.sendMessage(`reminder: !abort will stop the map, and !panic will suspend the auto-lobby process and notify organizers ${abortText}`);
    ...
    mpLobby.on("allPlayersReady", async () => {
        ...
        await mpChannel.sendMessage(`as a reminder, !abort will stop the map, and !panic will suspend the auto-lobby process and notify organizers ${abortText}`);
        await mpLobby.startMatch(5);
        mapTimerStarted = true;
    });
